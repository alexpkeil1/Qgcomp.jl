...
@testset "Continouous outcomes" begin
    ...
    @test isapprox(coef(mint_nonlin_boot)[3], coef(mint_nonlin_boot.ulfit)[3], atol=0.0001) && !isapprox(coef(mint_nonlin_boot)[2], coef(mint_nonlin_boot.ulfit)[2], atol=0.0001) && isapprox(coef(mint_nonlin_boot)[2], coef(mint_nonlin_boot.ulfit)[2]+coef(mint_nonlin_boot.ulfit)[5], atol=0.0001)
    @test isapprox(coef(mint_nonlin_ee)[3], mint_nonlin_ee.ulfit.cols[1][3], atol=0.0001) && !isapprox(coef(mint_nonlin_ee)[2], mint_nonlin_ee.ulfit.cols[1][2], atol=0.0001) && isapprox(coef(mint_nonlin_ee)[2], mint_nonlin_ee.ulfit.cols[1][2]+mint_nonlin_ee.ulfit.cols[1][5], atol=0.0001)

    println("Testing display: non linear")
    ...
    @test isapprox(coef(mint_nonlinx_boot)[3], coef(mint_nonlinx_boot.ulfit)[end], atol=0.0001) && !isapprox(coef(mint_nonlinx_boot)[2], coef(mint_nonlinx_boot.ulfit)[2], atol=0.0001) && isapprox(coef(mint_nonlinx_boot)[2], coef(mint_nonlinx_boot.ulfit)[2]+coef(mint_nonlinx_boot.ulfit)[3], atol=0.0001)
    @test isapprox(coef(mint_nonlinx_ee)[3], mint_nonlinx_ee.ulfit.cols[1][8], atol=0.0001) && !isapprox(coef(mint_nonlinx_ee)[2], mint_nonlinx_ee.ulfit.cols[1][2], atol=0.0001) && isapprox(coef(mint_nonlinx_ee)[2], mint_nonlinx_ee.ulfit.cols[1][2]+mint_nonlinx_ee.ulfit.cols[1][3], atol=0.0001)
...
@testset "Binary outcomes" begin
    ...
    # test 1c (ee model): does specifying breaks give the same answer as leaving unspecified when the expected answer is the same?
    mint_equiv_ee = qgcomp_glm_ee( form, logitdata, expnms, 4, Binomial(), msmformula = @formula(y~mixture+x2+z1+z2+z3), breaks = reduce(hcat, brks), )
    @test isapprox(sort(coef(mint_ee)), sort(coef(mint_equiv_ee)), atol = 0.001)
...
@testset "Survival outcomes" begin
    ...
    # test 7: no intercept (need to modify here)
    msg = try 
