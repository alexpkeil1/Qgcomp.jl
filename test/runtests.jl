...
@testset "Qgcomp.jl" begin
    ...
    logit_mint = qgcomp_glm_noboot(form, logitdata, expnms, 4, Binomial())
    logit_mnoint = qgcomp_glm_noboot(form_noint, logitdata, expnms, 4, Bernoulli())


    mnointnolin = qgcomp_glm_boot(form_noint_nonlin, lindata, expnms, 4, Normal())
    mnointnolin = qgcomp_glm_boot(form_noint_nonlin, lindata, expnms, 4, Normal(), degree = 2)
    mnointnolin2 = qgcomp_glm_boot(form_noint_nonlin, lindata,expnms,4, Normal(),
        msmformula = @formula(y~-1 + mixture + mixture^2 + x2 + z1 + z2 + z3)
    )
    ...
    logitident_mnointnolin =  qgcomp_glm_boot(form_noint_nonlin, logitdata, expnms, 4, Binomial(), msmlink=IdentityLink())
    logit_mnointnolin =  qgcomp_glm_boot(form_noint_nonlin, logitdata, expnms, 4, Binomial(), degree = 2);
    logit_mnointnolin2 = qgcomp_glm_boot(form_noint_nonlin, logitdata, expnms, 4, Binomial(),
        msmformula = @formula(y~-1 + mixture + mixture^2 + x2 + z1 + z2 + z3)
    )
    ...
    mcox2boot_alt = qgcomp_cox_boot(
        @formula(Surv(out, d)~z1+z2+x),
        survdata,
        ["z1", "z2"],
        4,
        B = 2,
        mcsize = 8000,
        msmformula = @formula(Surv(out, d)~mixture+x)
    )
    ...
    println(logit_mint)
    println(logit_mint.ulfit)

    println(mnoint)
    println(mnoint.ulfit)

    println(logit_mnoint)
    println(logit_mnoint.ulfit)

    println(mnointnolin)
    println(mnointnolin.ulfit)

    println(logit_mnointnolin)
    println(logit_mnointnolin.ulfit)

    println(mnointnolin2)
    println(mnointnolin2.ulfit)

    println(logit_mnointnolin2)
    println(logit_mnointnolin2.ulfit)
